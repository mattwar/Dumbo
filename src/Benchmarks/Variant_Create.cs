...
namespace Benchmarks
{
    public class Variant_Create
    {
        ...
        private void Test<T>(T value)
        {
            ...
            {
                v = Variant.Create(value);
            }
        }

        [Benchmark]
        public void Variant_Create_Int()
        {
            Test(1);
        }

        [Benchmark]
        public void Variant_Create_Enum()
        {
            Test(UI32Enum.A);
        }

        [Benchmark]
        public void Variant_Create_Nullable_Int()
        {
            Test<int?>(1);
        }

        [Benchmark]
        public void Variant_Create_Decimal_Small()
        {
            Test(1.0m);
        }

        [Benchmark]
        public void Variant_Create_String()
        {
            Test("one");
        }

        [Benchmark]
        public void Variant_Create_ReferenceType()
        {
            Test(new ReferenceType("one", 1));
        }

        [Benchmark]
        public void Variant_Create_SmallStructMixed()
        {
            Test(new SmallStructMixed("one", 1));
        }

        [Benchmark]
        public void Variant_Create_WrapperStruct()
        {
            Test(new WrapperStruct("one"));
        }

        [Benchmark]
        public void Variant_Create_SmallStructNoRefs()
        {
            Test(new SmallStructNoRefs(1, 2));
        }

        [Benchmark]
        public void Variant_Create_LargeStructNoRefs()
        {
            Test(new LargeStructNoRefs(1, 2, 3, 4));
        }

        [Benchmark]
        public void Variant_Create_LargeStructMixed()
        {
